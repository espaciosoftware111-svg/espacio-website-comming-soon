  ...
  return (
    ...
    >
      ...
      <header className="relative z-30 w-full bg-transparent pt-3 sm:pt-4 lg:pt-5 pb-1 shrink-0">
        <div className="w-full px-3 sm:px-6 md:px-8 lg:px-12 xl:px-14 h-14 sm:h-16 lg:h-18 flex items-center justify-between">
          
          {/* Left: Brand Monogram & Title with Localized Ethereal Mist */}
          <div
            className="relative flex items-center gap-2.5 sm:gap-3.5 lg:gap-4 select-none px-2 sm:px-3 py-1 sm:py-1.5 cursor-pointer group transition-all duration-300"
            onMouseEnter={() => setIsLogoHovered(true)}
            onMouseLeave={() => setIsLogoHovered(false)}
          >
            {/* Luminous Mist — Scoped strictly behind logo */}
            <div
              className="absolute -inset-x-3 sm:-inset-x-6 -inset-y-2 sm:-inset-y-3.5 pointer-events-none -z-10 filter blur-lg opacity-[0.97]"
              style={{
                background:
            ...
            {/* Monogram Emblem (Crisp, bold, prominent 38px on mobile, scaling to 52px on desktop) */}
            <div className={`w-[36px] h-[36px] xs:w-[40px] xs:h-[40px] sm:w-[44px] sm:h-[44px] md:w-[48px] md:h-[48px] lg:w-[52px] lg:h-[52px] flex items-center justify-center transition-all duration-300 shrink-0 ${isLampLit ? 'scale-110 drop-shadow-[0_0_16px_rgba(223,194,141,0.7)]' : 'group-hover:scale-105'}`}>
            ...
            {/* Typography Lockup - Bold, prominent, and clearly readable */}
            <div className="inline-flex flex-col items-start justify-center pl-0.5 sm:pl-1 overflow-visible">
              {/* ESPACIO */}
              <span className="font-['Montserrat',sans-serif] text-[15px] xs:text-[16.5px] sm:text-[18px] md:text-[20px] lg:text-[22px] font-bold tracking-[0.20em] xs:tracking-[0.24em] sm:tracking-[0.30em] text-[#121316] leading-none whitespace-nowrap drop-shadow-[0_1px_4px_rgba(255,255,255,0.95)]">
                ESPACIO
              </span>
              ...
              <span className="font-['Montserrat',sans-serif] text-[6.5px] xs:text-[7.2px] sm:text-[7.8px] md:text-[8.5px] lg:text-[9.5px] tracking-[0.20em] xs:tracking-[0.24em] sm:tracking-[0.34em] font-semibold text-[#121316] uppercase leading-none whitespace-nowrap drop-shadow-[0_1px_4px_rgba(255,255,255,0.95)]">
                INTERIORS AND MODULAR
              </span>
            </div>
          </div>
